...
- Implement a time-based `FrameCache` service that owns `loadFrame`, `getFrame`, eviction, and metrics. Keep a configurable sliding window of timeline frames (e.g., previous/current/next) in memory so seeking stays smooth.
- Each cache entry stores its typed-array/geometry buffers, `byteSize`, `status`, and `lastAccess`. Sum all buffer sizes—including derived Three.js attributes—to track exact usage against `MAX_FRAME_CACHE_MB`.
- Allow the app to read a `MAX_FRAME_CACHE_MB` from the query params (default 512) to emulate constrained hardware.
- Expose cache status via CLI/env/UI with the included frames info. Respect the hard cap (evict old loads once `currentUsageMb + newSize > limitMb`) and raise a soft warning as the budget is approached.

### Navigation Controls
- Implement WASD key controls to move the camera parallel to the ground plane of the scene.
- Implement Q/E key controls to move the camera along the vertical axis (perpendicular to the ground plane).
- Implement arrow key (↑←↓→) controls to rotate and pan the camera view.
